...
class words_with_Z{
    ...
    non_stop_words;
    stop_words;  
    constructor(wfapp:WordFrequencyFramework,wfCounter:WordFrequencyCounter,data_storage:DataStorage){
        wfapp.register_for_end_event(this.print_no_stop_words.bind(this))
        this.wfCounter=wfCounter
        this.ds=data_storage
    }
    print_no_stop_words(){
        this.non_stop_words=Object.keys(this.wfCounter.word_freq).length
        this.stop_words=this.ds.data.split(' ').length-this.non_stop_words;
        console.log("Stop Words count in the file= "+this.stop_words+" z")
    }
}
...
let wors_z=new words_with_Z(wfapp, word_freq_counter,data_storage)
wfapp.run("in.txt")