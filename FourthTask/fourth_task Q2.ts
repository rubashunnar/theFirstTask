...
//main
let file_data=read_file("in.txt");
let normalized=filter_chars_and_normalize(file_data);
let scanned=scan(normalized);
let removed=remove_stop_words(scanned)
let freq=frequencies(removed)
let sorted=sort(freq);
for (let i=0; i<25; i++){
