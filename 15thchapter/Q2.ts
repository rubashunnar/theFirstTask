...
class words_with_Z{ 
    ...
    ds;
    wfCounter;
    constructor(event_manager,ds,wfCounter){
       this.ds=ds
       this.wfCounter=wfCounter
       this.em=event_manager;
       this.em.subscribe('print',this.print_no_stop_words.bind(this))
    }
    print_no_stop_words(event){
        console.log(this.ds.data.split(' ').length-Object.keys(this.wfCounter.word_freq).length)
    }
}
